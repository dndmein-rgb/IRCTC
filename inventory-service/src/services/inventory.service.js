...
export const initializeInventory  = async (eventData) => {
  ...
  logger.info(`Inventory initialized for schedule ${scheduleId} with ${totalSeats} seats`);
       ...
       try {
            ...
            logger.error('Failed to publish initial availability event after retries', { scheduleId, error: err instanceof Error?err.message:String(err) });
       }
};

export const cancelScheduleInventory = async (eventData) => {
     ...
     try {
          ...
          logger.error('Failed to publish cancellation availability event after retries', { scheduleId, error:err instanceof Error? err.message:String(err) });
     }
};
