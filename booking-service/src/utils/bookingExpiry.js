...
async function tryAcquireLeadership() {
     try {
          ...
          logger.error('Failed to acquire expiry job leadership', { error: err instanceof Error?err.message:String(err) });
          return false;
     }
}
...
async function cleanExpiredBookings() {
     ...
     try {
          ...
          logger.error('Error in booking expiry job', { error:error instanceof Error?error.message:String(error) });
     }
}
